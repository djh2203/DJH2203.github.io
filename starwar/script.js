...
var SHIP_SPEED = 8; // 自機のスピード
var UFO_SPEED = 10; // UFO のスピード
var ITEM_SPEED = 3; // アイテムのスピード
...
function tweetLink() {
    ...
    if (isWap) {
        if (!dom.tweet) {
            ...
            dom.tweet.id = 'tweet';
            dom.tweet.innerHTML = '了解我们-青岛九中软件工程社';
        }
        dom.tweet.href = 'http://qd09soft.top/';
    } else {
...
Asteroid.prototype = extend(new Point(), {
    ...
    damage: function (damage, splinters) {
        ...
        if (this.radius < ASTEROID_MIN_SIZE) {
            ...
            // 進行角度を最大 30 度ずらす
            this.angle += DEG_TO_RAD * randUniform(70, -70);
            this.make();
        }
